...
module Searchable
  ...
  included do
    ...
    __elasticsearch__.client = Elasticsearch::Client.new(node: (ENV['ELASTICSEARCH_URL'] || 'http://localhost:9200'), log: true, trace: true)
    ...
    def self.es_version
      @es_version ||= __elasticsearch__.client.info["version"]["number"]
    end
      ...
      @search_definition = {
        ...
        aggregations: {
          ...
          published: {
            filter: { bool: { must: [ match_all: {} ] } },
            aggregations: {
              published: { date_histogram: { field: 'published_on', "#{Article.es_version.to_i == 7 ? 'calendar_interval' : 'interval'}" => 'week' } }
            }
          }
        }
      }
